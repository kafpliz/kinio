...
export class SearchService {
    ...
    async getData(q: string, type: string, page: number | null) {
        ...
        if (type == "user") {
            const user = AppDataSource.getRepository(User).createQueryBuilder("user").take(this.limit).skip(this.skip)
            ...
            const person = AppDataSource.getRepository(Person).createQueryBuilder("person").take(this.limit).skip(this.skip)
